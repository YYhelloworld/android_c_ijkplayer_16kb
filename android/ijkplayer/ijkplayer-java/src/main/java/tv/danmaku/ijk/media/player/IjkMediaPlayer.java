    ...
    public static class DefaultMediaCodecSelector implements OnMediaCodecSelectListener {
        ...
        public String onMediaCodecSelect(IMediaPlayer mp, String mimeType, int profile, int level) {
            ...
            if (bestCodec.mRank < IjkMediaCodecInfo.RANK_LAST_CHANCE) {
                Log.w(TAG, String.format(Locale.US, "unaccetable codec: %s rank=%d, trying fallback", bestCodec.mCodecInfo.getName(), bestCodec.mRank));
                // Fallback: try to find the first non-software hardware decoder
                for (IjkMediaCodecInfo codec : candidateCodecList) {
                    String name = codec.mCodecInfo.getName().toLowerCase(Locale.US);
                    if (!name.startsWith("omx.google.") && !name.startsWith("omx.ffmpeg.") && !name.startsWith("omx.avcodec.")) {
                        Log.i(TAG, String.format(Locale.US, "fallback selected codec: %s rank=%d", codec.mCodecInfo.getName(), codec.mRank));
                        return codec.mCodecInfo.getName();
                    }
                }
                // Last resort: return the best ranked codec anyway
                Log.i(TAG, String.format(Locale.US, "last resort codec: %s rank=%d", bestCodec.mCodecInfo.getName(), bestCodec.mRank));
