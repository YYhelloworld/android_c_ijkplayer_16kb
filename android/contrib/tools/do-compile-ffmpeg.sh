...
echo "--------------------"
export PATH=$IJK_NDK_BIN:$PATH

# Use Clang directly from NDK
export CC=${FF_CROSS_PREFIX}${FF_API_LEVEL}-clang
export CXX=${FF_CROSS_PREFIX}${FF_API_LEVEL}-clang++
export LD=ld.lld
export AR=llvm-ar
export AS=${FF_CROSS_PREFIX}${FF_API_LEVEL}-clang
...
# Pass explicit tool paths to FFmpeg configure (instead of --cross-prefix)
...
FF_CFG_FLAGS="$FF_CFG_FLAGS --ld=$CC"
...
# Add sysroot to cflags and ldflags
FF_CFLAGS="$FF_CFLAGS --sysroot=$IJK_NDK_SYSROOT"
FF_EXTRA_LDFLAGS="$FF_EXTRA_LDFLAGS --sysroot=$IJK_NDK_SYSROOT"
...
$CC -lm -lz -shared --sysroot=$IJK_NDK_SYSROOT -Wl,--no-undefined -Wl,-z,noexecstack $FF_EXTRA_LDFLAGS \
