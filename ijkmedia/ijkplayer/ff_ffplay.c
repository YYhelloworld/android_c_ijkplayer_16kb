...
#undef isnan
#endif
#define isnan(x) (__builtin_isnan((double)(x)) || __builtin_isnan((float)(x)))
#endif
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            if (ffp->no_sync) {
            /* no_sync mode: display frame immediately, skip all sync logic */
            delay = 0;
            is->frame_timer = av_gettime_relative() / 1000000.0;
            ...
            if (ffp->show_status) {
                ALOGI("ijkplayer: no_sync video frame pts=%f", vp->pts);
            }
            } else {
...
static int audio_decode_frame(FFPlayer *ffp)
{
    ...
    if (ffp->no_sync) {
        /* no_sync mode: skip sync_av_start, start audio immediately */
    } else if (ffp->sync_av_start &&                       /* sync enabled */
...
static int read_thread(void *arg)
{
    ...
    /* open the streams */
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0 && !ffp->no_sync) {
        /* no_sync mode: skip audio stream to save CPU and reduce latency */
    ...
    if (ffp->no_sync) {
        /* no_sync mode: override infinite_buffer to 0 to limit read-ahead buffer */
        ffp->infinite_buffer = 0;
        /* Override buffer limits for live low-latency streaming */
        /* Reduce max_buffer_size to 64KB for ultra-low latency */  
        ffp->dcc.max_buffer_size = 64 * 1024;
        /* Reduce min_frames to 1, minimal pre-reading */
        ffp->dcc.min_frames = 1;
        /* Set high water mark to 10ms for near real-time buffering */
        ffp->dcc.first_high_water_mark_in_ms = 10;
        ffp->dcc.next_high_water_mark_in_ms = 20;
        ffp->dcc.last_high_water_mark_in_ms = 50;
        ffp->dcc.current_high_water_mark_in_ms = 10;
        ALOGI("ijkplayer: no_sync mode enabled, buffer limits: max_buffer_size=%dKB, min_frames=%d, hwm=%dms",
            ffp->dcc.max_buffer_size / 1024, ffp->dcc.min_frames, ffp->dcc.first_high_water_mark_in_ms);
    }
...
static int video_refresh_thread(void *arg)
{
    ...
    while (!is->abort_request) {
        if (ffp->no_sync) {
            /* no_sync mode: poll as fast as possible, no sleep */
            remaining_time = 0.0;
            if (ffp->show_status) {
                static int64_t last_log = 0;
                int64_t now = av_gettime_relative();
                if (now - last_log > 5000000) { // log every 5s
                    ALOGI("ijkplayer: video_refresh_thread no_sync mode active");
                    last_log = now;
                }
            }
        } else {
