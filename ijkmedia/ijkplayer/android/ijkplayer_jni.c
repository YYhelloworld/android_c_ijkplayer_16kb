...
static bool mediacodec_select_fallback(JNIEnv *env, ijkmp_mediacodecinfo_context *mcc)
{
    // Direct fallback using MediaCodecList JNI when Java callback fails
    jclass cls_MediaCodecList = (*env)->FindClass(env, "android/media/MediaCodecList");
    if (!cls_MediaCodecList) {
        (*env)->ExceptionClear(env);
        return false;
    }

    jmethodID mid_getCodecCount = (*env)->GetStaticMethodID(env, cls_MediaCodecList, "getCodecCount", "()I");
    jmethodID mid_getCodecInfoAt = (*env)->GetStaticMethodID(env, cls_MediaCodecList, "getCodecInfoAt", "(I)Landroid/media/MediaCodecInfo;");
    if (!mid_getCodecCount || !mid_getCodecInfoAt) {
        (*env)->ExceptionClear(env);
        (*env)->DeleteLocalRef(env, cls_MediaCodecList);
        return false;
    }

    jint numCodecs = (*env)->CallStaticIntMethod(env, cls_MediaCodecList, mid_getCodecCount);
    ...
    // Get method IDs for MediaCodecInfo
    jclass cls_MediaCodecInfo = (*env)->FindClass(env, "android/media/MediaCodecInfo");
    if (!cls_MediaCodecInfo) {
        (*env)->ExceptionClear(env);
        (*env)->DeleteLocalRef(env, cls_MediaCodecList);
        return false;
    }

    jmethodID mid_isEncoder = (*env)->GetMethodID(env, cls_MediaCodecInfo, "isEncoder", "()Z");
    jmethodID mid_getName = (*env)->GetMethodID(env, cls_MediaCodecInfo, "getName", "()Ljava/lang/String;");
    jmethodID mid_getSupportedTypes = (*env)->GetMethodID(env, cls_MediaCodecInfo, "getSupportedTypes", "()[Ljava/lang/String;");
    if (!mid_isEncoder || !mid_getName || !mid_getSupportedTypes) {
        (*env)->ExceptionClear(env);
        (*env)->DeleteLocalRef(env, cls_MediaCodecList);
        (*env)->DeleteLocalRef(env, cls_MediaCodecInfo);
        return false;
    }

    jstring j_mimeType = (*env)->NewStringUTF(env, mcc->mime_type);
    if (!j_mimeType) {
        (*env)->ExceptionClear(env);
        (*env)->DeleteLocalRef(env, cls_MediaCodecList);
        (*env)->DeleteLocalRef(env, cls_MediaCodecInfo);
    ...
    bool found = false;
    for (jint i = 0; i < numCodecs && !found; i++) {
        jobject j_codecInfo = (*env)->CallStaticObjectMethod(env, cls_MediaCodecList, mid_getCodecInfoAt, i);
        if ((*env)->ExceptionCheck(env) || !j_codecInfo) {
            (*env)->ExceptionClear(env);
            continue;
        }

        jboolean isEncoder = (*env)->CallBooleanMethod(env, j_codecInfo, mid_isEncoder);
        if ((*env)->ExceptionCheck(env)) {
            (*env)->ExceptionClear(env);
            (*env)->DeleteLocalRef(env, j_codecInfo);
            continue;
        }
        if (isEncoder) {
            (*env)->DeleteLocalRef(env, j_codecInfo);
        ...
        jobjectArray j_types = (jobjectArray)(*env)->CallObjectMethod(env, j_codecInfo, mid_getSupportedTypes);
        if ((*env)->ExceptionCheck(env) || !j_types) {
            (*env)->ExceptionClear(env);
            (*env)->DeleteLocalRef(env, j_codecInfo);
            continue;
        }

        jsize typeCount = (*env)->GetArrayLength(env, j_types);
        for (jsize t = 0; t < typeCount && !found; t++) {
            jstring j_type = (jstring)(*env)->GetObjectArrayElement(env, j_types, t);
            if (!j_type) continue;

            const char *c_type = (*env)->GetStringUTFChars(env, j_type, NULL);
            if (c_type) {
                if (strcasecmp(c_type, mcc->mime_type) == 0) {
                    // Found a matching decoder - get its name
                    jstring j_codecName = (jstring)(*env)->CallObjectMethod(env, j_codecInfo, mid_getName);
                    if (!(*env)->ExceptionCheck(env) && j_codecName) {
                        const char *c_codecName = (*env)->GetStringUTFChars(env, j_codecName, NULL);
                        if (c_codecName) {
                            strlcpy(mcc->codec_name, c_codecName, sizeof(mcc->codec_name));
                            ALOGI("%s: fallback found codec: %s\n", __func__, mcc->codec_name);
                            found = true;
                            (*env)->ReleaseStringUTFChars(env, j_codecName, c_codecName);
                        }
                        (*env)->DeleteLocalRef(env, j_codecName);
                    } else {
                        (*env)->ExceptionClear(env);
                    }
                }
                (*env)->ReleaseStringUTFChars(env, j_type, c_type);
            }
    ...
    (*env)->DeleteLocalRef(env, j_mimeType);
    (*env)->DeleteLocalRef(env, cls_MediaCodecInfo);
    (*env)->DeleteLocalRef(env, cls_MediaCodecList);
...
static bool mediacodec_select_callback(void *opaque, ijkmp_mediacodecinfo_context *mcc)
{
    ...
    if (J4A_ExceptionCheck__catchAll(env) || !found_codec_name) {
        ALOGE("%s: onSelectCodec failed, trying fallback\n", __func__);
        if (mediacodec_select_fallback(env, mcc)) {
            found_codec_name = mcc->codec_name;
        } else {
            ALOGE("%s: fallback also failed\n", __func__);
        }
    }
