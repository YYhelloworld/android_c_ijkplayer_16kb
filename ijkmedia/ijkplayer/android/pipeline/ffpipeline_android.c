...
static IJKFF_Pipenode *func_open_video_decoder(IJKFF_Pipeline *pipeline, FFPlayer *ffp)
{
    ...
    if (ffp->mediacodec_all_videos || ffp->mediacodec_avc || ffp->mediacodec_hevc || ffp->mediacodec_mpeg2) {
        ALOGI("ijkplayer: trying MediaCodec decoder (mediacodec_all_videos=%d, avc=%d, hevc=%d, mpeg2=%d)",
            ffp->mediacodec_all_videos, ffp->mediacodec_avc, ffp->mediacodec_hevc, ffp->mediacodec_mpeg2);
        node = ffpipenode_create_video_decoder_from_android_mediacodec(ffp, pipeline, opaque->weak_vout);
    }
    if (!node) {
        ALOGI("ijkplayer: falling back to ffplay software decoder");
        node = ffpipenode_create_video_decoder_from_ffplay(ffp);
    } else {
        ALOGI("ijkplayer: using MediaCodec decoder successfully");
    }
